// Sound reactive version wooooooooo
...
function setup() {
  ...
  ui_button_audio = createButton("Enable Mic");
  ui_button_audio.position(35, 10);
  ui_button_audio.mousePressed(() => {
    mic.start(
      () => {
...
function draw() {
  if (backgroundUpdate) { clearCanvas(); }

  // Audio stuffs
  if(mic && mic.enabled) {
    /* let level = amplitude.getLevel(); // returns ~0.0 to 0.4 normally
    let spectrum = fft.analyze();
    let bass = fft.getEnergy("bass");
    let treble = fft.getEnergy("treble");

    entropy.set_pushAmount(map(level, 0, 0.3, 0.001, 0.03));
    entropy.set_opacity(map(bass, 0, 255, 10, 150));
    entropy.set_baseSpread(map(treble, 0, 255, 100, 500));

    console.log("audio:", { level, bass, treble }); */


    let vol = mic.getLevel();
    console.log("mic.getLevel():", vol);
  }
...
function _entropyBundleConfig(_def_) {
  ...
  if (typeof _def_ === 'object' && _def_ != null) {
    ...
    obj.controllers = {
      ...
      opacity: { val: 50, min: 2, max: 255, step: 1 },
      baseSpread: { val: 200, min: 1, max: 1000, step: 1 },
